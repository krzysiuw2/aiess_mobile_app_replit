"""
15-Minute Aggregation Lambda Function for InfluxDB Cloud Serverless
...
This function:
1. Runs every 15 minutes (triggered by EventBridge)
2. Queries the previous 15 minutes of 1-minute aggregated data from aiess_v1_1m
3. Aggregates to 15-minute intervals using SQL
4. Writes aggregated data to aiess_v1_15m bucket
"""
...
SOURCE_BUCKET = "aiess_v1_1m"
TARGET_BUCKET = "aiess_v1_15m"
...
def lambda_handler(event, context):
    """
    Lambda handler for 15-minute aggregation.
    Triggered by EventBridge every 15 minutes.
    """
    print(f"Starting 15-minute aggregation at {datetime.now(timezone.utc).isoformat()}")

    try:
        # Calculate time window: previous 15 minutes
        now = datetime.now(timezone.utc)
        window_end = now.replace(minute=(now.minute // 15) * 15, second=0, microsecond=0)
        ...
        # Query 1-minute aggregated data and aggregate to 15 minutes
        query = f"""
        SELECT
            site_id,
        ...
        # Query last active rule from the 1m bucket for this 15-min window
        rule_info = {}
        try:
            rule_query = f"""
