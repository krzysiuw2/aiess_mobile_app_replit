...
const ROSS_K = 0.0342;        // Ross model constant — "not so well cooled" mounting
 ...
 */
export function calculateArrayPower(gtiWm2, tempAmbientC, array) {
...
export function calculateArrayPowerWithShading(gtiWm2, diffuseWm2, directWm2, tempAmbientC, array, isShaded) {
  if (!isShaded) {
    return calculateArrayPower(gtiWm2, tempAmbientC, array);
  }
  ...
  return calculateArrayPower(effectiveGti, tempAmbientC, array);
}
 ...
 */
export function calculateSitePv(weatherByOrientation, arrays, mode = 'all', acCapacityKw = null) {
  ...
  for (const array of filteredArrays) {
    ...
    for (const row of weatherRows) {
      const power = calculateArrayPower(row.gti, row.temp, array);
  ...
  for (const [time, totalKw] of timeMap) {
    result.push({
      time,
      pvKw: Math.min(totalKw, acClampW / 1000),
    });
  }
