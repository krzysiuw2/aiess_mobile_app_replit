 ...
 * and monthly summaries from DynamoDB (via edge proxy, when available).
 */
...
// ─── Monthly Summaries (DynamoDB — not yet deployed) ────────────

/**
 * Fetch monthly financial summaries from DynamoDB via edge proxy.
 * Returns empty array until the financial-engine Lambda is deployed.
 */
export async function fetchMonthlyFinancialSummary(
  ...
  // TODO: Implement when financial-engine Lambda is deployed
  // Will fetch from DynamoDB via edge proxy
  console.log(`[Financial] Monthly summary fetch: site=${siteId}, year=${year}, month=${month}`);
  return [];
}
