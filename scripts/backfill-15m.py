"""
Backfill 15-minute aggregated data from 1m data.
Reads aiess_v1_1m, writes to aiess_v1_15m.
"""
...
GAP_START = datetime(2026, 3, 5, 8, 45, tzinfo=timezone.utc)
...
def main():
    ...
    print(f"Backfilling 15m aggregations from {GAP_START.isoformat()} to {GAP_END.isoformat()}")
    ...
    try:
        ...
        if table.num_rows == 0:
            print("No 1m data found for the gap period.")
            return
        ...
        for i in range(table.num_rows):
            ...
            points.append({
                ...
                "tags": {"site_id": site_id, "aggregation": "15m"},
                "fields": fields,
                "time": time_val,
            })
        ...
        print(f"Backfill complete! Wrote {total} 15m points.")
    finally:
        client.close()
